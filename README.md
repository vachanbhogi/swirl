...
QuaZe is the active application at the repository root. Run `jac start` from
the root to launch it.

The latest Swirl source that this branch was reconciled against is preserved in
[`swirl/`](swirl/) for reference and future feature porting. It is not the
default application, and no second copy of QuaZe is nested inside it.
